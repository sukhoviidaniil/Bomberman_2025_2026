...
namespace bomberman::logic {
    ...
    std::shared_ptr<Character> HeadlessEntityFactory::make_character(
        const CharacterKind kind, const sif::math::Point2 position, const float size, const float speed) {
        return std::make_shared<Character>(
            kind == CharacterKind::Player ? "Player" : "Bot", position, size, speed, kind);
    }

    std::shared_ptr<Bomb> HeadlessEntityFactory::make_bomb(
    ...
    std::shared_ptr<Explosion> HeadlessEntityFactory::make_explosion(
        const sif::math::Point2 position, const float size, const TilePos cell,
