...
Direction escape_after_bomb(const BotContext& ctx, const TilePos& from) {
    const DangerMap hypothetical = ctx.danger.with_bomb(ctx.grid, from, ctx.self.blast_radius(), ctx.bomb_fuse_seconds);
    ...
    const std::optional<TilePos> refuge = PathFinder::find_nearest(ctx.grid, from, passable, goal, escape_search_depth);
