...
namespace bomberman::logic::ai {
    ...
    BotPersonality personality_from_string(const std::string &value) {
        if (value == "balanced")   return BotPersonality::Balanced;
        if (value == "aggressive") return BotPersonality::Aggressive;
        if (value == "collector")  return BotPersonality::Collector;
        throw std::runtime_error(
            "unknown bot personality '" + value + "' (expected balanced, aggressive or collector)");
    }

    std::string to_string(const BotPersonality personality) {
        switch (personality) {
            case BotPersonality::Aggressive: return "aggressive";
            case BotPersonality::Collector:  return "collector";
