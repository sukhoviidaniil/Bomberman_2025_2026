...
bool DangerMap::safe(const TilePos& cell) const { return seconds_until_blast(cell) == never; }

bool DangerMap::safe_for(const TilePos& cell, const float seconds) const { return seconds_until_blast(cell) > seconds; }

std::size_t DangerMap::index(const TilePos& cell) const {
    return static_cast<std::size_t>(cell.row) * static_cast<std::size_t>(columns_) + static_cast<std::size_t>(cell.col);
}
