...
bool Character::can_place_bomb() const { return alive_ && bombs_placed_ < bomb_budget_; }

void Character::on_bomb_placed() { ++bombs_placed_; }
...
void Character::set_power_up_rules(const PowerUpRules& rules) { rules_ = rules; }

const PowerUpRules& Character::power_up_rules() const { return rules_; }
...
bool Character::may_pass(const TilePos& cell) const { return has_pass_cell_ && pass_cell_ == cell; }

void Character::forget_leaving() { has_pass_cell_ = false; }

void Character::set_obstacle_check(std::function<bool(const TilePos&)> check) { obstacle_check_ = std::move(check); }
