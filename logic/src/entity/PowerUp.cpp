...
PowerUp::PowerUp(const sif::math::Point2 position, const float size, const TilePos cell, const PowerUpKind kind,
    ...
    : Entity("PowerUp", position, size), cell_(cell), kind_(kind), shield_remaining_(std::max(0.f, shield_seconds)) {}
...
bool PowerUp::shielded() const { return shield_remaining_ > 0.f; }
