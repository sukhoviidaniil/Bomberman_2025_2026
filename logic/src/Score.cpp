...
namespace bomberman::logic {
    ...
    Score::Score(const std::shared_ptr<sif::event::Event_Bus>& world_bus, const ScoreRules rules) : rules_(rules) {

        track(world_bus->subscribe<game_events::Tick>([this](const game_events::Tick& e) {
            seconds_alive_ += e.dt;
        ...
        track(world_bus->subscribe<game_events::BlockDestroyed>([this](const game_events::BlockDestroyed& e) {
            if (!e.by_player) {
        ...
        track(world_bus->subscribe<game_events::PowerUpTaken>([this](const game_events::PowerUpTaken& e) {
            if (!e.by_player) {
                return;
            }
            ++power_ups_taken_;
        ...
        track(world_bus->subscribe<game_events::CharacterKilled>([this](const game_events::CharacterKilled& e) {
        ...
        track(world_bus->subscribe<game_events::RoundEnded>([this](const game_events::RoundEnded& e) {
    ...
    int Score::seconds_alive() const {
        return whole_seconds_awarded_;
    }
    int Score::blocks_destroyed() const {
        return blocks_destroyed_;
    }
    int Score::power_ups_taken() const {
        return power_ups_taken_;
    }
    int Score::enemies_killed() const {
        return enemies_killed_;
    }
    const ScoreRules& Score::rules() const {
        return rules_;
    }
} // namespace bomberman::logic
