...
GameConfig GameConfig::load(const std::string& filepath) {
    ...
    config.round.character_speed = sif::io::get_checked<float>(round, "character_speed", config.round.character_speed);
    config.round.character_size = sif::io::get_checked<float>(round, "character_size", config.round.character_size);
    ...
    config.round.bot_count = sif::io::get_checked<std::size_t>(round, "bot_count", config.round.bot_count);
    ...
    if (round.contains("bot_personalities") && !round.at("bot_personalities").is_null()) {
        ...
        for (const auto& entry : round.at("bot_personalities")) {
            ...
            config.round.bot_personalities.push_back(ai::personality_from_string(entry.get<std::string>()));
        }
    }

    const nlohmann::json power_ups = section(j, "power_ups");
    config.power_ups.drop_chance = sif::io::get_checked<float>(power_ups, "drop_chance", config.power_ups.drop_chance);
    config.power_ups.fire_weight = sif::io::get_checked<float>(power_ups, "fire_weight", config.power_ups.fire_weight);
    ...
    config.power_ups.max_speed = sif::io::get_checked<float>(power_ups, "max_speed", config.power_ups.max_speed);

    const nlohmann::json score = section(j, "score");
    config.score.per_second_alive = sif::io::get_checked<int>(score, "per_second_alive", config.score.per_second_alive);
    config.score.per_block_destroyed =
        sif::io::get_checked<int>(score, "per_block_destroyed", config.score.per_block_destroyed);
    config.score.per_power_up = sif::io::get_checked<int>(score, "per_power_up", config.score.per_power_up);
    config.score.per_enemy_killed = sif::io::get_checked<int>(score, "per_enemy_killed", config.score.per_enemy_killed);
    config.score.win_bonus = sif::io::get_checked<int>(score, "win_bonus", config.score.win_bonus);
    config.score.loss_penalty = sif::io::get_checked<int>(score, "loss_penalty", config.score.loss_penalty);
    ...
    config.audio.master_volume = sif::io::get_checked<float>(audio, "master_volume", config.audio.master_volume);
    config.audio.sfx_volume = sif::io::get_checked<float>(audio, "sfx_volume", config.audio.sfx_volume);
    ...
    if (!map.layout.empty()) {
        ...
        for (std::size_t i = 0; i < map.layout.size(); ++i) {
            if (map.layout[i].size() != width) {
                throw std::runtime_error("config: map.layout row " + std::to_string(i) + " is " +
