...
AudioDirector::AudioDirector(const std::shared_ptr<sif::event::Event_Bus>& world_bus, sif::audio::AudioPlayer& audio,
    ...
    track(world_bus->subscribe<logic::game_events::BombPlaced>([this](const logic::game_events::BombPlaced& e) {
    ...
    track(world_bus->subscribe<logic::game_events::BombExploded>([this](const logic::game_events::BombExploded&) {
    ...
    track(world_bus->subscribe<logic::game_events::PowerUpTaken>(
        [this](const logic::game_events::PowerUpTaken& e) { play(assets::sfx_pickup, e.by_player ? 1.f : 0.4f); }));

    track(
        world_bus->subscribe<logic::game_events::CharacterKilled>([this](const logic::game_events::CharacterKilled& e) {
            play(assets::sfx_death, e.victim == logic::CharacterKind::Player ? 1.f : 0.6f);
        }));

    track(world_bus->subscribe<logic::game_events::RoundEnded>([this](const logic::game_events::RoundEnded& e) {
...
void AudioDirector::play(const std::string& asset_name, const float volume_scale) const {
    ...
    audio_.play(assets_.sound(asset_name), std::clamp(config_.sfx_volume * volume_scale, 0.f, 1.f));
}
} // namespace bomberman::view
