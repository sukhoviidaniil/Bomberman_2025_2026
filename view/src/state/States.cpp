...
namespace bomberman::view {
    ...
    void LevelState::on_enter(StateManager& manager) {
        ...
        audio_ =
            std::make_unique<AudioDirector>(world_bus_, manager.game().audio(), manager.game().assets(), config.audio);
    ...
    const logic::Score* LevelState::score() const {
        return score_.get();
    }
    ...
    bool PausedState::draws_below() const {
        return true;
    }
    ...
    void SaveScoreState::refresh(StateManager& manager) {
        ...
        scene_.set_text("board_note",
                        qualifies ? "this would make the top five" : "this is not enough for the top five");
        scene_.set_color("board_note",
                         qualifies ? sif::intrnl::Color(120, 220, 150) : sif::intrnl::Color(150, 155, 175));
    ...
    bool SaveScoreState::draws_below() const {
        return true;
    }
} // namespace bomberman::view
