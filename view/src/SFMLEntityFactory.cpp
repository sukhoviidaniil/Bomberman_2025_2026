...
namespace bomberman::view {
    ...
    std::shared_ptr<logic::Character> SFMLEntityFactory::make_character(
        const logic::CharacterKind kind, const sif::math::Point2 position,
        const float size, const float speed) {

        auto model = std::make_shared<logic::Character>(
            kind == logic::CharacterKind::Player ? "Player" : "Bot", position, size, speed, kind);
    ...
    std::shared_ptr<logic::Bomb> SFMLEntityFactory::make_bomb(
        const sif::math::Point2 position, const float size, const logic::TilePos cell,
        std::weak_ptr<logic::Character> owner, const unsigned int radius, const float fuse_seconds) {
    ...
    std::shared_ptr<logic::Explosion> SFMLEntityFactory::make_explosion(
        const sif::math::Point2 position, const float size, const logic::TilePos cell,
