...
SFMLEntityFactory::SFMLEntityFactory(ViewRegistry& views, const GameAssets& assets) : views_(views), assets_(assets) {}
...
std::shared_ptr<logic::Character> SFMLEntityFactory::make_character(const logic::CharacterKind kind,
    ...
    auto model = std::make_shared<logic::Character>(kind == logic::CharacterKind::Player ? "Player" : "Bot", position,
                                                    size, speed, kind);
...
std::shared_ptr<logic::Explosion> SFMLEntityFactory::make_explosion(const sif::math::Point2 position, const float size,
                                                                    const logic::TilePos cell,
