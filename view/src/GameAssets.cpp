...
GameAssets::GameAssets() {
    ...
    for (const std::string& name :
         {ui_font,        player_walk_up,   player_walk_down, player_walk_left,  player_walk_right,
          player_idle_up, player_idle_down, player_idle_left, player_idle_right, player_die,
          bomb,           explosion,        tile_floor,       tile_wall,         tile_block,
          item_fire,      item_bomb,        item_skates,      sfx_explosion,     sfx_bomb_place,
          sfx_pickup,     sfx_death,        sfx_menu,         sfx_victory,       sfx_defeat}) {
        request(name);
    }
}
...
sif::intrnl::GUID GameAssets::guid_of(const std::string& name) {
    try {
        ...
        throw std::runtime_error("asset '" + name + "' is not in the registry (" + e.what() +
...
sif::asset::AssetHandle<sif::asset::PrimitiveAnimation> GameAssets::player_walk(
    const logic::Direction direction) const {
    switch (direction) {
    case logic::Direction::Up:
        return animation(player_walk_up);
    case logic::Direction::Left:
        return animation(player_walk_left);
    case logic::Direction::Right:
        return animation(player_walk_right);
    default:
        return animation(player_walk_down);
    }
}

sif::asset::AssetHandle<sif::asset::PrimitiveAnimation> GameAssets::player_idle(
    const logic::Direction direction) const {
    switch (direction) {
    case logic::Direction::Up:
        return animation(player_idle_up);
    case logic::Direction::Left:
        return animation(player_idle_left);
    case logic::Direction::Right:
        return animation(player_idle_right);
    default:
        return animation(player_idle_down);
    }
}

sif::asset::AssetHandle<void> GameAssets::tile(const logic::Tile t) const {
    switch (t) {
    case logic::Tile::Indestructible:
        return sprite(tile_wall);
    case logic::Tile::Destructible:
        return sprite(tile_block);
    default:
        return sprite(tile_floor);
    }
}

sif::asset::AssetHandle<void> GameAssets::item(const logic::PowerUpKind kind) const {
    switch (kind) {
    case logic::PowerUpKind::ExtraBomb:
        return sprite(item_bomb);
    case logic::PowerUpKind::Skates:
        return sprite(item_skates);
    default:
        return sprite(item_fire);
    }
}
} // namespace bomberman::view
